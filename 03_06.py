...
def simulate_pg_agent(agent_cls, price_data: np.ndarray, **kwargs):
    ...
    wealth = [1000.0]
    actions = []  # Collect weights over time

    # Start equal weights
    w = np.ones(n_assets, dtype=float) / n_assets
    actions.append(w.copy())  # Initial weights
    ...
    for t in range(1, n_steps):
        ...
        w = b_t
        actions.append(w.copy())  # Collect after update
...
def run_portfolio_analysis(portfolio_spec: dict):
    ...
    asset_labels = [a.split('.')[0] for a in assets]
    ...
    for algo_name, Algo in SB3_ALGOS.items():
        ...
        if algo_name in ["DDPG", "DDPG_PINN"]:  # Incremental for DDPG and DDPG_PINN to show convergence
            total_steps = TRAIN_STEPS_MAP[algo_name]
            ...
            for milestone in range(1, num_milestones + 1):
                model.learn(steps_per_milestone)
                print(f"{algo_name} trained for {milestone * steps_per_milestone} steps ({milestone * 10} 'episodes').")

                # Evaluate on validation set
                val_env = MultiAssetPortfolioEnv(val_df, assets, FEATURES, window_size)
                wealth, actions = simulate_sb3_strategy(val_env, model, return_actions=True)
                actions = np.array(actions)  # (timesteps, n_assets)
                avg_weights = np.mean(actions, axis=0)
                episode_label = milestone * 10
                avg_weights_dict[episode_label] = avg_weights
                wealth_curves[episode_label] = wealth

                # Compute cumulative return for this milestone
                cum_ret = ((wealth[-1] / wealth[0]) - 1) * 100 if wealth else 0.0
                cumulative_returns.append(cum_ret)

            # Plot cumulative returns (existing)
            plt.figure(figsize=(16, 8))
            for label, wealth in wealth_curves.items():
                plt.plot(wealth, label=f"Episode {label}")
            plt.title(f"Cumulative Returns of {algo_name} against Training Episode on Validation Set - {name}")
            plt.xlabel("Trading Days in Validation Set")
            plt.ylabel("Portfolio Value ($)")
            plt.legend(ncol=2, fontsize=10)
            plt.tight_layout()
            plt.savefig(f"cumulative_returns_{algo_name}_{name}.png")
            plt.close()

            # Optional: Plot cum ret % vs episodes (existing)
            plt.figure(figsize=(10, 6))
            episodes = [i * 10 for i in range(1, num_milestones + 1)]
            plt.plot(episodes, cumulative_returns, marker='o', linestyle='-')
            plt.title(f"Cumulative Return (%) vs Training Episodes for {algo_name} on Validation Set - {name}")
        ...
        # Test evaluation for all (final model)
        eval_env = MultiAssetPortfolioEnv(test_df, assets, FEATURES, window_size)
        wealth, actions = simulate_sb3_strategy(eval_env, model, return_actions=True)
        results[algo_name] = wealth
        actions_dict[algo_name] = actions
        wealth_data[algo_name] = wealth
    ...
    for label, (cls, params) in pg_agents.items():
        ...
        if label == "BEST":
            params["price_history"] = test_close
        wealth, actions = simulate_pg_agent(cls, test_close, **params)
        results[label] = wealth
        actions_dict[label] = actions
        wealth_data[label] = wealth
        metrics_rows.append({"Algorithm": label, **compute_metrics(wealth)})

    all_algos = list(SB3_ALGOS.keys()) + list(pg_agents.keys())
    for algo in all_algos:
        if algo in actions_dict:
            act = np.array(actions_dict[algo])  # (timesteps, n_assets)
            os.makedirs(algo, exist_ok=True)
            plt.figure(figsize=(16, 8))
            for i, lab in enumerate(asset_labels):
                plt.plot(act[:, i], label=lab)
            plt.title(f"Portfolio Weights over Test Period - {algo} - {name}")
            plt.xlabel("Trading Days")
            plt.ylabel("Weight")
            plt.legend(ncol=3, fontsize=10)
            plt.tight_layout()
            plt.savefig(f"{algo}/weights_changes_{algo}_{name}.png")
            plt.close()
    
    # Step 6: For point 3, save all test wealth data as CSV
    # Insert after the weight changes loop
    max_len = max(len(w) for w in wealth_data.values()) if wealth_data else 0
    df_wealth = pd.DataFrame(index=range(max_len))
    for algo, w in wealth_data.items():
        df_wealth[algo] = pd.Series(w).reindex(range(max_len))
    df_wealth.to_csv(f"all_test_wealth_{name}.csv", index_label="Step")
    ...
    end_time = time.time()
    print(f"\nTotal analysis time for {name}: {end_time - start_time:.2f} seconds")

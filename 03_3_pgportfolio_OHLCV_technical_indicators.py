...
def simulate_pg_agent(agent_cls, price_data: np.ndarray, **kwargs):
    """
    Step pgportfolio agents correctly: pass 1×N price-relative vector each step.
    - UBAH: buy-and-hold (no rebalancing) -> weights drift with returns.
    - Others (CRP/OLMAR/RMR/PAMR): rebalanced using agent's output.
    """
    ...
    for t in range(1, n_steps):
        ...
        if agent_cls.__name__.upper() == "UBAH":
            # Buy-and-hold: DO NOT rebalance; just let weights drift
            b_t = w  # use current holdings
        else:
            # Rebalancing agent: ask for new target weights given the CURRENT rel
            try:
            ...
            if b_t.size != n_assets or not np.all(np.isfinite(b_t)):
                b_t = w  # fallback
            # Some algos may output negatives / not sum to 1 -> fix
            if (b_t < 0).any() or not np.isclose(b_t.sum(), 1.0):
                b_t = _simplex_project(b_t)

        # Apply period return using the *pre-return* allocation b_t
        step_ret = float(np.dot(b_t, rel))
        wealth.append(wealth[-1] * step_ret)

        # Holdings drift with returns regardless of whether we rebalanced this step
        w = b_t * rel
        w = w / w.sum()  # normalize to get end-of-period holdings for next step
